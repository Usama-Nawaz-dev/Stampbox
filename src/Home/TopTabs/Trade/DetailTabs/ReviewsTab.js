...
export const ReviewsTab = (props) => {
    ...
    const fetchComments = async () => {
        ...
        const response = await MindAxios.get(
            Env.createUrl(
                `comments?commentable_id=${tradeData?.id}&commentable_type=Trade`
            )
        );
    ...
    const nextPageComments = async () => {
        const response = await MindAxios.get(nextApiUrl + `&commentable_id=${tradeData?.id}&commentable_type=Trade`);
        if (response?.status === 200) {
            const _data = response?.data?.result?.paginated_items?.data;
            const _nextPageUrl = response?.data?.result?.paginated_items?.next_page_url;
            // console.log(_data, "next.....", _nextPageUrl);
    ...
    const postComment = async () => {
        if (commentText?.length) {
            dispatch(allActions.DataAction.AppLoader(true));
            const body = {
                commentable_type: 'Trade',
                commentable_id: tradeData?.id,
                message: commentText,
            };
        ...
        <View style={[styles.container, { backgroundColor: theme?.white}]}>
            {commentList?.length ?
                ...
                <View style={styles.mainItem}>
                    <AppText style={[styles.itemText, { color: theme?.theme}]}>Trade don't have any reviews.</AppText>
                </View>
            }
                    ...
                    <View style={styles.bottomSheetView}>
                        ...
                        >
                            <View style={styles.bottomSheetOptionView}>
                                <Feather name="edit" size={30} color={theme?.black} />
                        ...
                        >
                            <View style={styles.bottomSheetOptionView}>
                                <AntDesign name="delete" size={30} color={theme?.black}/>
