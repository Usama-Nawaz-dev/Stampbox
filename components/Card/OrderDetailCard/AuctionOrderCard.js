...
export const AuctionOrderCard = (props) => {
    const { orderDetail, auction } = props;
    const stampUri = auction?.medias[0]?.media_url;
    const senderUri = orderDetail?.sender?.image_url;
    const receiverUri = orderDetail?.receiver?.image_url;
    const shipping = orderDetail?.order_meta?.shipping_address;
    const total = parseFloat(orderDetail?.subtotal) + parseFloat(orderDetail?.shipment?.amount);
        ...
        <View style={[styles.card, { backgroundColor: theme?.cardColor}]}>
            <View style={styles.headerSection}>
                <View style={styles.rowSection}>
                    ...
                    <AppText style={styles.packageText}>Package</AppText>
                </View>
            ...
            <View style={styles.topSection}>
                ...
                <View style={styles.userSection}>
                    <AppText style={styles.nameText}>{auction?.name}</AppText>
                    <AppText style={[styles.infoText, { color: theme?.lightText}]}>Traking Code: {orderDetail?.shipment?.tracking_code}</AppText>
                    <AppText style={[styles.infoText, { color: theme?.lightText}]}>Carrier: {orderDetail?.shipment?.carrier}</AppText>
                    <AppText style={[styles.infoText, { color: theme?.lightText}]}>Service: {orderDetail?.shipment?.service}</AppText>
                </View>
            </View>
            ...
            <View style={styles.orderSection}>
                <View style={{ justifyContent: 'space-between' }}>
                    <Image source={senderUri ? { uri: senderUri } : images.Profile} style={styles.userImg} />
            ...
            <View style={styles.addressSection}>
                <View style={styles.priceSection}>
                    ...
                    <AppText style={styles.amountText}>${orderDetail?.subtotal}</AppText>
                </View>
                <View style={[styles.priceSection, { borderTopWidth: 0 }]}>
                    <AppText style={styles.priceText}>Shipping Charges:</AppText>
                    <AppText style={[styles.priceText, { fontWeight: '400' }]}>${orderDetail?.shipment?.amount}</AppText>
                </View>
            </View>
