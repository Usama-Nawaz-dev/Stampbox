...
export const ProductReview = (props) => {
  ...
  const productDetail = useSelector((state) => state.DetailReducer.stampDetail);
  ...
  const fetchComments = async () => {
    ...
    const response = await MindAxios.get(
      Env.createUrl(
        `comments?commentable_id=${productDetail?.id}&commentable_type=Product`
      )
    );
  ...
  const nextPageComments = async () => {
    const response = await MindAxios.get(
      nextApiUrl + `&commentable_id=${productDetail?.id}&commentable_type=Trade`
    );
    if (response?.status === 200) {
      const _data = response?.data?.result?.paginated_items?.data;
      const _nextPageUrl =
  ...
  const postComment = async () => {
    if (commentText?.length) {
      ...
      const body = {
        ...
        commentable_id: productDetail?.id,
        message: commentText,
      };
