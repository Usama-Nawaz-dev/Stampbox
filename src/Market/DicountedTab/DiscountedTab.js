...
export const DiscountedTab = ({ navigation }) => {
  ...
  const getSales = async () => {
    const response = await MindAxios.get(
      Env.paramUrl(`products`, `?discounted_items=true&is_published=2`)
    );
  ...
  const fetchMoreSales = async () => {
    if (page < endPage && salesList?.length >= 10) {
      ...
      const response = await MindAxios.get(
        Env.paramUrl(
          `products`,
          `?discounted_items=true&is_published=2&page=${page + 1}`
        )
      );
      setLoading(false);
      const error = response?.response?.data;
  ...
  const onApplyFilter = async () => {
    ...
    const response = await MindAxios.get(
      Env.paramUrl(
        `products`,
        `?discounted_items=true&is_published=2&page_size=9&${query}`
      )
    );
    dispatch(allActions.DataAction.AppLoader(false));
    if (response?.status === 200) {
  ...
  const onSearchData = async (x) => {
    if (x) {
      ...
      const response = await MindAxios.get(
        Env.paramUrl(
          `products`,
          `?discounted_items=true&is_published=2&page_size=9&search=${x}`
        )
      );
      setIsLoading(false);
      if (response?.status == 200) {
