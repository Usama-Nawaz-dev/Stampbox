...
import { View, TouchableOpacity, Image, FlatList, Modal, Switch } from 'react-native'
...
import {
    MainHeader, FloatingInput, BottomSheet,
    CustomButton, GradBtn
} from '../../../../components';
...
export const EditBounty = (props) => {
    ...
    arrayChecker(staticGrades, ItemDetail?.grade);
    ...
    arrayChecker(staticFormat, ItemDetail?.format);
    const myData = Helper.deepCopy(qualityData)?.map((item) => {
        item.isSelected = false;
        return item;
    });
    arrayChecker(myData, ItemDetail?.quality);
    arrayChecker(staticCondition, ItemDetail?.condition);

    let optionType;
    if (ItemDetail?.offered_amount > 0 && ItemDetail?.offered_coins > 0) {
        optionType = "Both";
    } else if (ItemDetail?.offered_coins > 0) {
        optionType = "In Coins";
    } else if (ItemDetail?.offered_amount > 0) {
        optionType = "Money Only";
    }

    const [title, setTilte] = useState(ItemDetail?.title)
    const [errMsgs, setErrMsgs] = useState({});
    const [yearIssued, setYearIssued] = useState(ItemDetail?.year_issued);
    const [description, setDescription] = useState(ItemDetail?.description);
    const [selectedCountry, setSelectedCountry] = useState(ItemDetail?.country);
    const [itemFormat, setItemFormat] = useState(staticFormat);
    const [condition, setCondition] = useState(staticCondition);
    const [rewardOption, setRewardOption] = useState(optionType);
    const [grade, setGrade] = useState(staticGrades);
    const [qData, setQData] = useState(myData);
    const [isLocal, setIsLocal] = useState(ItemDetail?.is_local ? true : false);
    const [coinVal, setCoinVal] = useState(ItemDetail?.offered_coins);
    const [amount, setAmount] = useState(ItemDetail?.offered_amount);
    ...
    const checkValidations = async (check) => {
        ...
        if (rewardOption === 'Both') {
            if (!coinVal || !amount) {
                ...
                handleError("Please enter a value greater than or equal to 1", "coinErrMessage");
                isValid = false;
            }
            ...
            if (parseInt(amount) < 1) {
                handleError("Please enter a value greater than or equal to 1", "amountErrMessage");
                isValid = false;
            }
        }
        ...
        if (isValid === false) {
            ...
            const response = await MindAxios.post(
                // Store Bounty -> Edit
                Env.paramUrl("bounties", ItemDetail?.id),
                body
            );
    ...
    return (
        <View style={[styles.container, { backgroundColor: theme?.white}]}>
            ...
            <KeyboardAwareScrollView
                ...
                <AppText style={[styles.mediaText, {
                    color: theme.darkGrey, marginTop: 10, marginBottom: -10
                }]}>
                    Stamp Detail
                </AppText>
                <FloatingInput
                    ...
                    error={
                        errMsgs.titleErrMessage ? errMsgs.titleErrMessage : false
                    }
                />
                <FloatingInput
                ...
                <InputButton
                    ...
                    placeHolder={selectedCountry ? selectedCountry : language?.selectCountry}
                ...
                />
                <CustomDropDown
                    data={[{ value: "In Coins" }, { value: "Money Only" }, { value: "Both" }]}
                ...
                />
                {(rewardOption === "Money Only" || rewardOption === "Both") && <FloatingInput
                    ...
                    onChangeText={(text) => {
                        ...
                        rewardOption === "Both" && setCoinVal(`${(parseFloat(text) * 100).toFixed(0)}`);
                    }}
                    error={
                        errMsgs.amountErrMessage ? errMsgs.amountErrMessage : false
                    }
                />}
                {(rewardOption === "In Coins" || rewardOption === "Both") && <FloatingInput
                    label="Offered Coins"
                    ...
                    error={
                        errMsgs.coinErrMessage ? errMsgs.coinErrMessage : false
                    }
                />}
                <FloatingInput
                    label="Description"
                ...
                <View style={styles.btnSection}>
                    <CustomButton bg={theme.white} style={{ borderWidth: 1, borderColor: theme.lightGrey }}
                        label="Draft" textColor={theme.dovGray} fontWeight="600"
                        width={wp(45)} height={45} fontSize={16}
                        onPress={() => checkValidations(false)}
                    />
                    <GradBtn label="Active" height={45} width={wp(42)}
