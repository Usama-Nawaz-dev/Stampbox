...
export const EditEvent = (props) => {
  ...
  const localDate = schedule.start_at_date;

  arrayChecker1(years, detail?.years);
  ...
  arrayChecker(staticTopics, detail?.topics);
  let countryId = 1;
  ...
  arrayChecker(staticCountries, detail?.countries);

  const itemMeeting = detail?.meeting_type.replace(/_/g, " ");
  const [eventType, setEventType] = useState(detail?.type);
  const [meetingType, setMeetingType] = useState(itemMeeting);
  const [eventDuration, setEventDuration] = useState(detail?.duration_type);
  ...
  const [description, setDescription] = useState(detail?.event_desc);
  const [instruction, setInstruction] = useState(detail?.participant_desc);
  const [ticketPrice, setTicketPrice] = useState(
    detail?.ticket_price?.toString()
  );
  const [link, setLink] = useState(detail?.external_link);
  ...
  const [isParking, setIsParking] = useState(
    detail?.is_parking_exist ? true : false
  );
  ...
  const [selectedDate, setSelectedDate] = useState(
    `${schedule?.start_at_date} to ${schedule?.end_at_date}`
  );
  const [isDatePickerVisible, setDatePickerVisibility] = useState(false);
  const [countriesData, setCountriesData] = useState(staticCountries);
  const scheduleDates = detail?.event_schedules?.map(
    (item) => item.start_at_date
  );
  const [recurringList, setRecurringList] = useState(scheduleDates);
  const [eventCategories, setEventCategories] = useState([]);
  const [exhibCatgories, setExhibCategories] = useState([]);
  const [expPickerVisible, setExpPickerVisible] = useState(false);
  const [expDate, setExpDate] = useState(
    detail?.expiry_time ? detail?.expiry_time : null
  );
  const {
    myState: { language },
  ...
  const fetchCategories = async () => {
    ...
    if (response?.status == 200) {
      ...
      const exhibCateg = newData?.filter((item) => {
        if (item?.key === "Exhibition") {
          item.isSelected = true;
          return item;
        }
      });
      arrayChecker(newData, detail?.categories);
    ...
    <View style={[styles.mainConatiner, { backgroundColor: theme?.white }]}>
      ...
      >
        ...
        <CustomDropDown
          ...
          label={eventType ? "Type" : `${language?.selectType}*`}
        ...
        <InputButton
          ...
          placeHolder={language?.selectCategories + "*"}
        />
        <ErrorMessage
        ...
        <InputButton
          ...
          placeHolder={language?.selectTopics}
        />
        ...
        {eventType === "Virtual Exhibition" && (
          <>
            ...
            <InputButton
              ...
              label={language?.country}
              placeHolder={language?.selectCountry}
            ...
            <InputButton
              ...
              placeHolder={language?.pickYears}
            />
          </>
        )}
        <FloatingInput
