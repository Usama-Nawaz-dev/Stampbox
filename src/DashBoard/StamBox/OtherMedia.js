...
export const OtherMedia = (props) => {
  ...
  const getMedias = async () => {
    ...
    const response = await MindAxios.get(
      Env.paramUrl("medias/user", `${userId}?linked_with=other`)
    );
  ...
  const getNextMedia = async () => {
    try {
      if (nextApiUrl !== null) {
        setIsLoading(true);
        const response = await MindAxios.get(
          nextApiUrl + `&linked_with=other&page_size=${Number(limit)}}`
        );
        setIsLoading(false);
  ...
  const onApplyFilter = async () => {
    ...
    const response = await MindAxios.get(
      Env.paramUrl(
        "medias/user",
        `${userId}?linked_with=other&page_size=${limit}`
      )
    );
    dispatch(allActions.DataAction.AppLoader(false));
    setMediaData(response?.data?.result?.media?.data);
        ...
        <MediaCard
          ...
          rightIcon={
            <MenuSheet
              ...
              options={[
                "Move to My Stamps (Actual)",
                "Move to My Stamps (Sample)",
                "Move to My Supplies",
                "Cancel",
              ]}
              actions={[
                () => onMoveMedia(item, "auctual"),
                () => onMoveMedia(item, "sample"),
                () => moveToSupply(item),
              ]}
            />
          }
  ...
  const onMoveMedia = async (item, type) => {
    dispatch(allActions.DataAction.AppLoader(true));
  ...
  return (
    <View style={[styles.container, { backgroundColor: theme?.white }]}>
      <TouchableOpacity
        style={styles.addBtn}
        onPress={() => props.navigation.navigate("AddItem", { type: "other" })}
      >
        <FastImage
          source={images.Add}
