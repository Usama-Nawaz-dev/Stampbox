...
export const HunterTab = (props) => {
  ...
  const fetchBountyReq = async () => {
    ...
    const response = await MindAxios.get(
      Env.createUrl(
        `bounty-offers?bounty_id=${bountyDetail?.id}&status=Hunting_request`
      )
    );
  ...
  const renderItem = ({ item, index }) => {
    ...
    let myItem = item;
    myItem.description = `${item?.user?.first_name} sent you bounty hunting Request.`;
    return (
