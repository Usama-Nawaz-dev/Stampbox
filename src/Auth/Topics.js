...
import { newTopics as topics } from "../../constant/staticData";
...
const Topics = ({ navigation }) => {
  ...
  const searchData = (x) => {
    ...
    let filteredName = topics.filter((item) => {
  ...
  const next = async () => {
    if (current?.length > 0) {
      ...
      var result = topics.filter(function (item1) {
      ...
      alert("Please Select at least 1 Topic to go Next!");
    }
  };
  ...
  const handleNotification = async () => {
    const body = {
      "notifications.interested-topics": isEnabled ? "1" : "0",
    };
      ...
      <View>
        ...
        <View style={styles.checkSection}>
          ...
          <AppText style={styles.notifyText}>
            Send me notifications for selected topics.
          </AppText>
        </View>
        <SearchFilter
          onTermChange={(text) => {
  ...
  return (
    <Pressable
      ...
      style={{
        ...
        width: wp(45),
        marginBottom:
          index == topics.length - 1 || index == topics.length - 2 ? 50 : 15,
