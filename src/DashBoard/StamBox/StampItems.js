...
export const StampItems = (props) => {
  ...
  const getMedias = async () => {
    ...
    const response = await MindAxios.get(
      Env.paramUrl("medias/user", `${userId}?is_stamp=1&type=actual`)
    );
  ...
  const getNextMedia = async () => {
    try {
      if (nextApiUrl !== null) {
        ...
        const response = await MindAxios.get(
          nextApiUrl +
            `&is_stamp=1&type=actual&page_size=${limit}&order_by=${order_by}`
        );
        setIsLoading(false);
  ...
  const onApplyFilter = async () => {
    ...
    let apiUrl = `${userId}?is_stamp=1&type=actual&page_size=${limit}&order_by=${order_by}`;
    if (selectedCountry) {
        ...
        <MediaCard
          ...
          rightIcon={
            <MenuSheet
              ...
              options={["Move to My Stamps (Sample)", "Cancel"]}
              actions={[() => onMoveMedia(item)]}
            />
          }
        />
      </View>
  ...
  return (
    <View style={[styles.container, { backgroundColor: theme?.white }]}>
      <TouchableOpacity
        style={styles.addBtn}
        onPress={() =>
          props.navigation.navigate("AddItem", { isStamp: 1, type: "actual" })
        }
      >
        <FastImage
          source={images.Add}
