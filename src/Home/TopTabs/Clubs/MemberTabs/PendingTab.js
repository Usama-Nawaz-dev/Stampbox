...
export const PendingTab = () => {
    ...
    const fetchClubMembers = async () => {
        ...
        const response = await MindAxios.get(
            Env.createUrl(`club-members?club_id=${clubDetail?.id}&status=Pending&page_size=20`)
        );
        setLoading(false);
    ...
    const requestAction = async (status, memberId) => {
    ...
    const renderItem = ({ item, index }) => {
        ...
        return (
            <UserInviteCard
                User={item?.user}
                invited={true}
                onPress={() => requestAction("Accepted", item?.id)}
                onCancelInvite={() => requestAction("Rejected", item?.id)}
            />
        );
    };
