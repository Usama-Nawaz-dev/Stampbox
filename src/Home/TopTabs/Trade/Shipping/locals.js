...
const checkValidations = async (
  ...
  if (isValid) {
    ...
    let { trade_id } = currentItem;
    let bodyNext = {};
    bodyNext.trade_id = trade_id;
    ...
    if (status == 200) {
      ...
      // console.log(rates);
      let sendParam = { currentItem, rates, bodyNext };
      navigation.navigate("ShipRate", { sendParam });
    } else {
      Helper.showToastMessage(e?.response?.data?.message, colors.danger);
    }
  }
};
