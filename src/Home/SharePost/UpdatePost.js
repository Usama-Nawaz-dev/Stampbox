...
export const UpdatePost = (props) => {
  ...
  const [postImg, setPostImage] = useState(data?.uri);
  ...
  useFocusEffect(
    useCallback(() => {
      ...
      setPostImage({ uri: data?.uri });
      setDescription(data?.des);
      let last = data?.uri?.charAt(data?.uri?.length - 1);
      if (last !== "g" && last !== "f" && last !== "p") {
        data?.uri && setIsVideo(true);
      }
  ...
  const hitPost = async () => {
    if (postImg?.uri || description) {
      ...
      formdata.append("type", "home_feed");
      ...
      fetch(Env.createUrl(`posts/${data?.id}`), {
        method: "POST",
        headers: headers,
        body: formdata,
      })
    ...
    <View style={[styles.container, { backgroundColor: theme?.white}]}>
      ...
      <View style={styles.modalHeader}>
        <View style={styles.leftSection}>
          ...
          <AppText style={styles.heading}>Update Post</AppText>
        </View>
        <TouchableOpacity onPress={hitPost}>
          <AppText style={styles.postText}>Update</AppText>
        </TouchableOpacity>
      </View>
