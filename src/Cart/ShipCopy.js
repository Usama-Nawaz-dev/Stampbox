...
const Ship = (props) => {
  ...
  const getRatesApi = async () => {
    ...
    if (check) {
      ...
      let store_Cart = localFuncs.storeCart(myCart?.myObj, to_address);
      console.log("store cart--->", store_Cart);
      const res = await MindAxios.post(
        Env.createUrl("shipments/generate-multi-rates"),
        store_Cart
      );
      ...
      if (res?.status == 200) {
        ...
        const newArr = result?.stores.map((item) => {
          return item;
        });
