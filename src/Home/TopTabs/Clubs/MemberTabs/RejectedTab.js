...
export const RejectedTab = () => {
    ...
    const fetchClubMembers = async () => {
        ...
        const response = await MindAxios.get(
            Env.createUrl(`club-members?club_id=${clubDetail?.id}&status=Rejected&page_size=20`)
        );
        setLoading(false);
    ...
    const inviteUser = async (memberId) => {
        const body = {
        ...
        if (response.status == 200) {
            const filterUsers = userList.filter((item) => item?.id !== memberId);
